...
EOT;
  ...
  {
    $replace = [':p_rtp_name' => $this->quoteText($pRtpName)];
    $query   = <<< EOT
select rsr.rsr_id
     , rsr.rsr_path
     , rsr.rsr_content_optimized
...
EOT;
  ...
  //--------------------------------------------------------------------------------------------------------------------
  /**
   * Selects the max depth of the resources.
   *
   * @return int
   */
...
EOT;
  ...
  {
    $replace = [':p_rsr_path' => $this->quoteText($pRsrPath)];
    $query   = <<< EOT
select rsr_id
     , rsr_path
     , rsr_mtime
...
EOT;
  ...
  {
    $replace = [':p_rsr_id' => $this->quoteInt($pRsrId),
                ':p_rsr_uri_optimized' => $this->quoteText($pRsrUriOptimized)];
    $query   = <<< EOT
update ABC_RESOURCE
